...
import { createProvider, getAutoProviderDescription } from "lib/agent/provider-factory";
import { OllamaProvider, RECOMMENDED_MODELS } from "lib/agent/providers/ollama";
...
const PROVIDERS: ProviderOption[] = [
  {
    type: "ollama",
    name: "Ollama (Local)",
    description: "Free. Runs on your machine. Data never leaves your computer.",
    isLocal: true,
    needsKey: false,
    icon: ServerIcon,
  },
  {
    type: "gemini",
    name: "Google Gemini",
    description: "Free tier: 15 requests/minute. Get a free key from Google AI Studio.",
    isLocal: false,
    needsKey: true,
    icon: KeyIcon,
  },
...
export function AgentSetup({ onClose }: { onClose: () => void }) {
  ...
  }, [selectedType, refreshOllamaModels]);
  ...
  const downloadingTasks = downloadManager?.state.tasks.filter(
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
      <button
        ...
        onClick={onClose}
      />
      <div className="absolute inset-y-0 right-0 flex w-full max-w-lg flex-col border-l border-gray-200 bg-white shadow-xl">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {/* Free AI status banner (shown when no BYO key configured) */}
          {!config && (
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-medium text-emerald-800">
                    {PROVIDERS.find((p) => p.type === config.type)?.name ?? config.type}
                  </p>
          ...
          {/* Advanced accordion — BYO key providers */}
          <button
            type="button"
          ...
          >
            ...
            <ChevronDownIcon
              className={cn("h-4 w-4 text-gray-500 transition-transform", showAdvanced && "rotate-180")}
            />
          </button>
              ...
              <div className="rounded-lg border border-gray-200 p-4">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-sm font-medium text-gray-700">
                    {ollamaDetected ? "Ollama Running" : "Detecting Ollama..."}
                  </h4>
                </div>
                {!ollamaDetected && (
          ...
          {/* API Key Setup (Gemini, OpenAI, Groq) */}
